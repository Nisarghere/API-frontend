...
export default function Navbar() {
  ...
  return (
    <nav
  className={`sticky top-0 z-50 grid grid-cols-2 items-center gap-y-2 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl transition-all duration-300 md:grid-cols-3 md:gap-y-0 ${
    scrolled ? "px-4 py-2 shadow-sm sm:px-6 md:px-8" : "px-4 py-3 sm:px-6 md:px-8"
  }`}
>
  <Link
    href={"/"}
    ...
    {isLoggedIn ? (
      <button
        ...
        className={`cursor-pointer rounded-full bg-red-500 font-semibold text-white transition-all duration-300 hover:bg-red-700 ${
          scrolled ? "px-4 py-1.5 text-xs sm:px-5 sm:text-sm" : "px-4 py-1.5 text-sm sm:px-5 sm:py-2 sm:text-base"
        }`}
      >
        Log-Out
      </button>
      ...
      <>
        <Link
          ...
          className={`rounded-full font-semibold text-slate-700 transition-all duration-300 hover:text-blue-600 ${
            scrolled ? "px-3 py-1.5 text-xs sm:px-4 sm:text-sm" : "px-3 py-1.5 text-sm sm:px-4 sm:py-2 sm:text-base"
          }`}
        >
          Sign In
        </Link>
